...
export async function getUserTotalLikesCount (userUid: string): Promise<number> {
  ...
  ].map(async ({ ref, contentLikesRef }) => await getLikesCountForContentType(userUid, ref, contentLikesRef))
...
export async function getUserPhotos (uid: string): Promise<Photo[]> {
  ...
  const photosPromise = firebasePhotos.map(async (photo) =>
    await getImageUrl(ImageType.PHOTO, photo.uid).then((url) => ({ ...photo, url } as Photo))
  )
