...
export function subscribeBatteries (onValueChange: (items: Battery[]) => void): Unsubscribe {
  return onValue(batteriesRef, (snapshot: DataSnapshot) => {
    const batteriesPromise: Array<Promise<Battery>> = new Array<Promise<Battery>>()
...
export async function saveAffiliate (batteryId: string, { name, link }: Affiliate) {
  return await set(child(child(child(batteriesRef, batteryId), 'affiliate'), name), link)
}
